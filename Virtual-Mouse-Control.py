import cv2
import numpy as np
# pyautogui - to control mouse pointer and functions
import pyautogui
import time

# pre defined ranges for blue. red and yellow colors in HSV
blue_range = np.array([[94, 68, 35], [134, 255, 255]])
yellow_range = np.array([[17, 125, 82], [57, 255, 255]])
red_range = np.array([[160, 4, 39], [200, 255, 255]])
...
def distance(c1, c2):
    dist = pow(pow(c1[0]-c2[0], 2) + pow(c1[1]-c2[1], 2), 0.5)
    return dist
...
def changeStatus(key):
    ...
    if key == ord('p'):
        ...
        print(' Use the bars to calibrate and press SPACE when done.')
...
def drawCentroid(vid, color_area, mask, showCentroid):
    ...
    image, contour, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    l = len(contour)
    area = np.zeros(l)

    for i in range(l):
        if color_area[0] < cv2.contourArea(contour[i]) < color_area[1]:
...
def performAction(yp, rc, bc, action, drag, perform):
    if perform:
        ...
        cursor[1] = 4*(yp[1]-120)
        if action == 'move':
            if 110 < yp[0] < 590 and 120 < yp[1] < 390:
                pyautogui.moveTo(cursor[0], cursor[1])
            elif yp[0] < 110 and 120 < yp[1] < 390:
                pyautogui.moveTo(8, cursor[1])
            elif yp[0] > 590 and 120 < yp[1] < 390:
                pyautogui.moveTo(1912, cursor[1])
            elif 110 < yp[0] < 590 and yp[1] < 120:
                pyautogui.moveTo(cursor[0], 8)
            elif 110 < yp[0] < 590 and yp[1] > 390:
